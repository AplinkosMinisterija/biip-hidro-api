...
exports.up = async function (knex) {
  await knex.schema
    .alterTable('events', function (table) {
      table.index('upperBasin');
      table.index('lowerBasin');
    })
    .dropViewIfExists('hydro_power_plant_statistics')
    .createMaterializedView('hydro_power_plant_statistics', (view) => {
      view.as(
        knex
          .select(
            'hpp.id',
...
exports.down = async function (knex) {
  await knex.schema
    .alterTable('events', function (table) {
      table.dropIndex('upperBasin');
      table.dropIndex('lowerBasin');
    })
    .dropMaterializedViewIfExists('hydro_power_plant_statistics')
